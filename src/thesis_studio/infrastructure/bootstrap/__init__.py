...
from ...application.writing import OutlineService, SectionWriter, TextPolisher
...
def get_section_writer() -> SectionWriter:
    """创建章节撰写服务。"""
...
def get_text_polisher() -> TextPolisher:
    """创建文本润色服务。"""
    return TextPolisher(llm=get_llm_provider())
...
class Services:
    ...
    @property
    def outline(self) -> OutlineService:
        return get_outline_service()

    @property
    def section_writer(self) -> SectionWriter:
        return get_section_writer()

    @property
    def text_polisher(self) -> TextPolisher:
        return get_text_polisher()
